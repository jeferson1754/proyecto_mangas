...
if ($variable) {
    ...
    $consulta1 = $conexion->prepare("SELECT * FROM `pendientes_manga` WHERE ID = ?");
    ...
    $sql = "SELECT Dia, COUNT(*) AS Cantidad, (SELECT AVG(Diferencia) FROM diferencias_pendientes WHERE ID_Pendientes = ?) AS PromedioGeneral 
            FROM diferencias_pendientes WHERE ID_Pendientes = ? GROUP BY Dia ORDER BY Cantidad DESC LIMIT 1";
    ...
    $sql1 = "SELECT * FROM `diferencias_pendientes` WHERE ID_Pendientes = ? ORDER BY `Fecha` DESC limit 50";
    ...
    $sql2 = "SELECT Fecha, Diferencia FROM `diferencias_pendientes` WHERE ID_Pendientes = ? ORDER BY `Fecha` DESC";
...
$sql_heatmap = "SELECT DAYOFWEEK(Fecha) as dia_semana, HOUR(Fecha) as hora, COUNT(*) as cantidad 
                FROM diferencias_pendientes WHERE ID_Pendientes = ? GROUP BY dia_semana, hora";
...
$sql_ultimo = "SELECT Fecha FROM diferencias_pendientes WHERE ID_Pendientes = ? ORDER BY Fecha DESC LIMIT 1";
